...
func App(readFile func(name string) ([]byte, error), args []string) (string, error) {
	...
	inner_brackets := `\[[^][]*\]|{[^}{]*}|\[.*\[.*\].*\]|\{.*\{.*\}.*\}`
	inner_element := `\s*(null|true|false|` + numbers + `|"[^\s"]+"|` + inner_brackets + `){1}`
	...
	object_key := `"[^\s"]+"\s*`
	last_element_in_outer_curly_brackets := `(\s*` + object_key + `:` + inner_element + `\s*)`
	multiple_elments_in_outer_curly_brackets := `(\s*` + object_key + `:` + inner_element + `\s*,\s*)*`
	outer_curly_brakets := `{\s*(` + multiple_elments_in_outer_curly_brackets + last_element_in_outer_curly_brackets + `{1}){0,1}}`
	regex := regexp.MustCompile(`(?s)\A\s*(` + outer_square_brackets + `|` + outer_curly_brakets + `){1}\s*\z`)
